Here is a structured, in-depth breakdown of the **Java Memory Model (JMM)** based directly on the provided transcript, organized chapter-by-chapter.
...
# HashSet (Interview Notes)

## 1. What is HashSet?

A **HashSet** is an implementation of the **Set** interface that stores **unique elements** using a **HashMap** internally.
...
HashSet doesn't have its own storage mechanism. It internally uses a **HashMap**.
...
HashSet relies on **`hashCode()`** to locate the bucket and **`equals()`** to check if the element already exists.
...
> Worst case is **O(log n)** after Java 8 due to Red-Black Tree buckets.
...
Because elements are stored as **keys** in a `HashMap`, and duplicate keys are not allowed.
...
* `equals()` checks whether the element already exists in that bucket.

---

### Does HashSet preserve insertion order?

No. Use **LinkedHashSet** if insertion order is required.

---

### Is HashSet sorted?

No. Use **TreeSet** if sorted order is required.
...
> **"HashSet is a Set implementation backed by a HashMap. It stores elements as keys with a dummy value (`PRESENT`) and uses `hashCode()` and `equals()` to ensure uniqueness."**
